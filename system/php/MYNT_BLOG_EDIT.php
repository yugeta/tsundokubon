...
class MYNT_BLOG_EDIT{
	...
	public static function setSystemPage(){
		...
		if($_REQUEST["mode"] === "remove" && isset($_REQUEST["file"]) && $_REQUEST["file"]){
			...
			header("Location: ". MYNT_URL::getUrl()."?system=blogLists&type=".$_REQUEST["type"]."&status=".$_REQUEST["status"]);
			exit();
		}
		...
		// source-save
		$_REQUEST["source"] = str_replace("\r\n","\n",$_REQUEST["source"]);
		$_REQUEST["source"] = str_replace("\r","\n",$_REQUEST["source"]);
